...
AnscriptLib Interface Suite
by Hothiptv/Anscript VN

Original by Anscript Hub

-------------------------------
AnscriptLib  | Designing + Programming + New Features
...
local AnscriptUIFolder = "AnscriptUI"
local ConfigurationFolder = AnscriptUIFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local AnscriptUIQuality = {}

local AnscriptLib = {
	Flags = {},
	Theme = {
		Default = {
			TextFont = "Default", -- Default will use the various font faces used across AnscriptUI
		...
		Light = {
			TextFont = "Gotham",  -- Default will use the various font faces used across AnscriptUI
...
local AnscriptUI = game:GetObjects("rbxassetid://11637506633")[1]

AnscriptUI.Enabled = false
-- Tasks
local spawn = task.spawn

--studio
if game["Run Service"]:IsStudio() then
	function gethui() return AnscriptUI end local http_request = nil local syn = {protect_gui = false,request = false,}local http = nil function writefile(tt,t,ttt)end function isfolder(t)end function makefolder(t)end function isfile(r)end function readfile(t)end
end

pcall(function()
_G.LastAnscriptUI.Name = 'Old AnscriptLib'
_G.LastAnscriptUI.Enabled = false
end)
local ParentObject = function(Gui)
	...
	_G.LastAnscriptUI = AnscriptUI
end
ParentObject(AnscriptUI)

--Object Variables

local Camera = workspace.CurrentCamera
local Main = AnscriptUI.Main
...
local BarType = 'Top'
local Notifications = AnscriptUI.Notifications

local SelectedTheme = AnscriptLib.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = AnscriptLib.Theme[ThemeName]
	for _, obj in ipairs(AnscriptUI:GetDescendants()) do
	...
	AnscriptUI.Main.BackgroundColor3 = SelectedTheme.Background
	AnscriptUI.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	AnscriptUI.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	AnscriptUI.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	AnscriptUI.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	AnscriptUI.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	AnscriptUI.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if AnscriptLib.Flags[FlagName] then
			spawn(function() 
				if AnscriptLib.Flags[FlagName].Type == "ColorPicker" then
					AnscriptLib.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if AnscriptLib.Flags[FlagName].CurrentValue or AnscriptLib.Flags[FlagName].CurrentKeybind or AnscriptLib.Flags[FlagName].CurrentOption or AnscriptLib.Flags[FlagName].Color ~= FlagValue then AnscriptLib.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			AnscriptLib:Notify({Title = "Flag Error", Content = "AnscriptUI was unable to find '"..FlagName.. "'' in the current script"})
		end
	end
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(AnscriptLib.Flags) do
...
function AnscriptLib:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= AnscriptLib.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("AnscriptUI | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
		...
		if AnscriptUI.Name == "AnscriptUI" then
...
function Hide()
	...
	Debounce = true
	AnscriptLib:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping RightShift", Duration = 7})
...
function AnscriptLib:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "AnscriptLib Interface Suite"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius | Meta"
	if Settings.LoadingTitle ~= "AnscriptLib Interface Suite" then
		LoadingFrame.Version.Text = "AnscriptLib"
	end
	...
	pcall(function()
		...
		if not isfolder(AnscriptUIFolder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(AnscriptUIFolder.."/Discord Invites") then
			makefolder(AnscriptUIFolder.."/Discord Invites")
		end
		if not isfile(AnscriptUIFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(AnscriptUIFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"AnscriptUI RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(AnscriptUIFolder.."/Key System") then
			makefolder(AnscriptUIFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			...
			if not Success then
				print("AnscriptUI | "..Settings.KeySettings.Key.." Error " ..tostring(Response))
			end
		end
		...
		if isfile(AnscriptUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			if readfile(AnscriptUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) == Settings.KeySettings.Key then
				Passthrough = true
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function(EnterPressed)
				...
				if KeyMain.Input.InputBox.Text == Settings.KeySettings.Key then
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(AnscriptUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, Settings.KeySettings.Key)
						end
						AnscriptLib:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
...
-- 	end)
	...
	AnscriptUIQuality.Window = {Tabs = {}}
	local Window = AnscriptUIQuality.Window
	function Window:CreateTab(Name,Image)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("AnscriptUI | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
		...
		function Tab:CreateSection(SectionName,Display)
			...
			if Display then
				...
				SectionValue.Holder.Parent = AnscriptUI.Holding
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function(enter)
				...
				if not Success then
					...
					print("AnscriptUI | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			local function AddOption(Option,Selecteds)
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("AnscriptUI | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
					end
			...
			local function AddOptions(Options,Selected)
				...
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
						AnscriptLib.Flags[DropdownSettings.Flag] = DropdownSettings
					end
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then  --Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("AnscriptUI | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					AnscriptLib.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("AnscriptUI | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					AnscriptLib.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					AnscriptLib.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
			...
			local function UpdateSlider(X)
				...
				if SliderSettings.CurrentValue ~= NewValue then
					...
					if not Success then
						...
						print("AnscriptUI | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					AnscriptLib.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
			...
			for name,info in pairs(PromptSettings.Actions) do
				...
				Button.Interact.MouseButton1Up:Connect(function()
					if not clicked then
						...
						if not Success then
							ClosePrompt()
							print("AnscriptUI | "..info.Name.." Callback Error " ..tostring(Response))
						else
							ClosePrompt()
						end
					end
...
function AnscriptLib:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				AnscriptLib:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end
task.delay(9, AnscriptLib.LoadConfiguration, AnscriptLib)
